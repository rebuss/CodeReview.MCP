...
| `REBUSS.Pure.Core\IFindingSourceProviderSelector.cs` | Feature 023 — picks an `IFindingSourceProvider` per review key. Local prefixes (`local:staged:`, `local:unstaged:`, `local:branch:`) map to a per-call wrapper bound to the matching git ref; all other review keys map to the singleton remote provider. Returned wrapper is review-scoped — callers must not cache across reviews. |
...
| `REBUSS.Pure\Services\CopilotReview\Validation\FindingSourceProviderSelector.cs` | Feature 023 — singleton `IFindingSourceProviderSelector` impl. Maps review-key prefix to source: `local:staged:` → `LocalGitClient.IndexRef`, `local:unstaged:` → `LocalGitClient.WorkingTreeRef`, `local:branch:` → `"HEAD"`, default → singleton `RemoteArchiveSourceProvider`. For local prefixes returns a per-call private `BoundLocalSourceProvider` wrapper carrying the bound git ref + a `_warnedRootMissing` boolean for FR-006 dedup. The wrapper is a value object (not DI-registered), so its mutable field is review-scoped, not service-scoped — Constitution §VI is preserved. |
