...
| `REBUSS.Pure.AzureDevOps\Names.cs` | Canonical Azure DevOps provider identifier constants (`Provider = "AzureDevOps"`, `ProviderLower = "azuredevops"`, `Domain = "dev.azure.com"`, `LegacyDomain = "visualstudio.com"`); referenced by `AzureDevOpsOptions.SectionName`, `Program.cs`, `McpWorkspaceRootProvider.cs`, `ProviderDetector.cs` via `AzureDevOpsNames` alias |
...
| File | Role | Depends on |
|---|---|---|
| `REBUSS.Pure.AzureDevOps\Providers\AzureDevOpsDiffProvider.cs` | Top-level orchestrator (~250 LOC after the Diff/* split) for PR diff fetching. Composes four collaborators (`PrDataFetcher`, `DiffSkipPolicy`, `DiffSourcePairFactory`, `IStructuredDiffBuilder`) and runs **one unified `Parallel.ForEachAsync`** (capped at `MaxParallelDiffRequests=15`) over the kept files. The API/ZIP strategy difference is hidden behind `IDiffSourcePair.ReadAsync`; the ZIP path's archive workspace is disposed via `await using` so the temp directory is always cleaned up. Per-file `BuildSingleFileDiff` consults `FullFileRewriteDetector` to skip files where every line changed (≥ 10 LOC). Populates `PullRequestDiff.LastSourceCommitId` from iteration's target commit for cache staleness detection. Public `GetDiffAsync` / `GetFileDiffAsync` keep `virtual` modifier for override surface; ctor is `internal` so the `Diff.*` collaborator types stay encapsulated. | `Diff.PrDataFetcher`, `IStructuredDiffBuilder`, `Diff.DiffSkipPolicy`, `Diff.DiffSourcePairFactory`, `PullRequestDiff`, `FileChange`, `DiffHunk` |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\PrDataFetcher.cs` | `internal sealed class PrDataFetcher(IAzureDevOpsApiClient, IPullRequestMetadataParser, IIterationInfoParser, IFileChangesParser)`. `FetchAsync(prNumber, ct)` returns the `internal readonly record struct PullRequestData(Metadata, Files, BaseCommit, TargetCommit)` after three sequential ADO API calls (PR details → last iteration → iteration changes when `iteration.Id > 0`). Sequential by design — iteration-changes depends on the iteration ID. Future opportunity: `details` + `iterations` could be `Task.WhenAll` (out of scope for the current refactor). |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\DiffSkipPolicy.cs` | `internal sealed class DiffSkipPolicy(IFileClassifier)`. `GetSkipReason(FileChange)` returns the resource-localized skip reason (`SkipReasonFileDeleted` / `SkipReasonFileRenamed` / `SkipReasonBinaryFile` / `SkipReasonGeneratedFile`) or `null` for files that should have their diff computed normally. Pure logic — no I/O — driven entirely by the change-type string and `IFileClassifier.Classify`. |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\IDiffSourcePair.cs` + `ApiDiffSourcePair.cs` + `ZipDiffSourcePair.cs` | Strategy seam for fetching the (base, target) content pair per file. `IDiffSourcePair : IAsyncDisposable` with one method `ReadAsync(path, ct)`. `ApiDiffSourcePair` runs two parallel `IAzureDevOpsApiClient.GetFileContentAtCommitAsync` calls; `DisposeAsync` is a no-op. `ZipDiffSourcePair` reads from a pre-extracted `ExtractedArchiveWorkspace` via `TryResolveFilePath` + `File.ReadAllText`; its `DisposeAsync` delegates to the workspace. Picked at runtime by `DiffSourcePairFactory` based on file count vs `ZipFallbackThreshold`. The only new abstraction in the diff refactor — chosen because it has 2 runtime-selectable implementations. |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\DiffSourcePairFactory.cs` | `internal sealed class DiffSourcePairFactory(IAzureDevOpsApiClient, AzureDevOpsRepositoryArchiveProvider, IOptions<AzureDevOpsDiffOptions>, ILogger)`. `CreateAsync(fileCount, baseCommit, targetCommit, ct)` returns `ApiDiffSourcePair` when `fileCount <= ZipFallbackThreshold` (or `threshold == 0`); otherwise downloads + extracts both archives synchronously (via `ExtractedArchiveWorkspace.CreateAsync`) and returns a `ZipDiffSourcePair` wrapping the workspace. Owns the "Using ZIP-fallback path …" Information log. |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\ExtractedArchiveWorkspace.cs` | `internal sealed class ExtractedArchiveWorkspace : IAsyncDisposable`. Encapsulates the lifecycle of `%TEMP%/rebuss-repo-{pid}/diff-{guid}/{base,target}/` — directory creation, parallel ZIP download via `AzureDevOpsRepositoryArchiveProvider.DownloadRepositoryZipAsync`, `ZipFile.ExtractToDirectory` × 2, ZIP delete-after-extract (free disk), and full temp-tree removal in `DisposeAsync`. Static `CreateAsync` factory does the synchronous up-front work and returns a ready-to-use workspace. `TryResolveFilePath(rootPath, diffFilePath)` handles both flat archives and single-wrapper-directory layouts (some ADO downloads nest under a top-level commit folder). Defence-in-depth `IsWithin` guard rejects paths that canonicalize outside the archive root. The parent `rebuss-repo-{pid}/` mirrors what `RepositoryCleanupService` uses, so a sibling sweep on next start can reclaim space if we crash mid-build. |
| `REBUSS.Pure.AzureDevOps\Providers\Diff\FullFileRewriteDetector.cs` | `internal static class FullFileRewriteDetector` with single method `IsFullFileRewrite(string? baseContent, string? targetContent, IReadOnlyList<DiffHunk> hunks)`. Pure predicate — both contents are non-trivial (≥ `FullRewriteMinLineCount = 10` lines) AND every line changed (deletions == old line count and additions == new line count). Files matching this pattern are skipped from the structured diff because the rebuilt hunks would be a wall of red+green lines that carry no signal beyond "the file was rewritten". |
...
| `REBUSS.Pure.RoslynProcessor\StructuralChangeDetector.cs` | Top-level orchestrator (~50 LOC after the Members/* + Type*/Base* split). Public `static class` with single entry `DetectChanges(SyntaxTree before, SyntaxTree after)` returning `IReadOnlyList<StructuralChange>` sorted by `LineNumber` (nulls last). Composes 6 collaborators: `TypeDeclarationIndex.BuildFrom` × 2 → `TypeChangeDetector.Detect` for added/removed types → loop over matched names → `BaseTypeChangeDetector.Detect` + `Members.MemberChangeDetector.Detect` → `SortByLineNumber`. Public surface unchanged from pre-refactor. | `TypeDeclarationIndex`, `TypeChangeDetector`, `BaseTypeChangeDetector`, `Members.MemberChangeDetector` |
| `REBUSS.Pure.RoslynProcessor\TypeDeclarationIndex.cs` | `internal sealed class TypeDeclarationIndex` with static `BuildFrom(SyntaxNode root)` factory + `Names` / indexer / `GetKindName` / `GetQualifiedName`. "Top-level types only" semantic — types declared directly under `BaseNamespaceDeclarationSyntax` or `CompilationUnitSyntax`; nested types deliberately excluded. Duplicate qualified names use first-wins semantics (preserves prior behaviour for partial classes). |
| `REBUSS.Pure.RoslynProcessor\TypeChangeDetector.cs` | `internal static` set diff over a pair of `TypeDeclarationIndex`. Emits `TypeAdded` (with kind name + LineNumber from after-state) and `TypeRemoved` (LineNumber=null — no after-state line to surface). |
| `REBUSS.Pure.RoslynProcessor\BaseTypeChangeDetector.cs` | `internal static` 3-branch decision over `BaseList?.Types` diff: full replacement (added+removed → "Base type changed: X → Y"), pure additions ("Implements added: …"), pure removals ("Implements removed: …"). Empty `BaseList` normalised to empty enumeration. |
| `REBUSS.Pure.RoslynProcessor\Members\MemberChangeDetector.cs` | `internal static` 4-phase algorithm: (1) build identity-key maps via `MemberKeyExtractor.Key`; (2) **orphan-pairing heuristic** — same-name+kind orphans with single match on both sides emit one `SignatureChanged` (sole overload had its parameter list edited), real overload additions/removals stay as add+remove; (3) emit `MemberAdded`/`MemberRemoved` for unpaired orphans; (4) emit `SignatureChanged` for matched-key pairs where `MemberSignatureComparer.AreEqual` returns false. |
| `REBUSS.Pure.RoslynProcessor\Members\MemberKeyExtractor.cs` | `internal static` identity key generators. `Key` (full identity including param-type list — distinguishes overloads) and `GroupKey` (name+kind only — used by orphan-pairing heuristic). Returns null from `Key` for unsupported member kinds. Covers method, ctor, property, field, indexer, operator, event, event-field. |
| `REBUSS.Pure.RoslynProcessor\Members\MemberSignatureComparer.cs` | `internal static` per-kind signature equality predicate. Method: param list + return type + modifiers + type params. Ctor: params + modifiers. Property: type + modifiers + accessor kinds. Field: type + modifiers. Indexer: param list + type + modifiers. Fallback: `SyntaxFactory.AreEquivalent`. Modifiers compared as ordered kind sets (so e.g. `static public` ≡ `public static`). |
| `REBUSS.Pure.RoslynProcessor\Members\MemberChangeFormatter.cs` | `internal static` user-visible description string builders. Per-kind switch for `FormatAdded` / `FormatRemoved` / `FormatSignatureChanged` (with `→` arrow). Owns `FormatParams` (truncation at >3 params: "type1, type2, ... +N") and `GetVisibility` (renders "Public " / "Protected " / "Internal " / "" prefix from modifier list, used only by `FormatRemoved`). |
...
| `REBUSS.Pure\Services\LocalReview\LocalReviewProvider.cs` | Orchestrates git client + unified-patch parser + file classifier. `GetAllFileDiffsAsync` runs one `git diff -p` per scope and parses with `UnifiedPatchParser`; `GetFileDiffAsync` is a thin filter on top. A short-lived (5 s TTL) single-slot snapshot cache keyed by `(repoRoot, scope.Kind, scope.BaseBranch)` makes the typical orchestrator-style `GetAllFileDiffsAsync` → repeated `GetFileDiffAsync` flow O(1) per file lookup; cache miss/expiry falls back to a fresh `git diff`. TTL bounds staleness because the provider is a DI singleton and the working tree may change between calls. Applies binary/generated skip reasons by classifier. No longer takes `IStructuredDiffBuilder` — local diffs are sourced from git's own output, not re-diffed in C#. | `IWorkspaceRootProvider` (from Core), `ILocalGitClient`, `IFileClassifier`, `UnifiedPatchParser`, domain models |
...
### Provider detection (REBUSS.Pure\ProviderDetection)

| File | Role |
|---|---|
| `REBUSS.Pure\ProviderDetection\ProviderDetector.cs` | Single source of truth for SCM provider selection. Public surface (used by `Program.ConfigureBusinessServices` → `DetectProvider`): `Detect(IConfiguration, string? repoPath)` — explicit `Provider` config (case-normalized) > `GitHub.Owner` > `AzureDevOps.OrganizationName` > git remote URL match > default `AzureDevOps`. Internal surface (used by `InitCommand.CreateAuthFlow` after Step 1 consolidation): `DetectFromGitRemote(string? workingDirectory)` runs `git remote get-url origin` in the supplied directory and delegates to the private `MapRemoteUrlToProvider(string?)` mapper that recognizes `github.com` → GitHub and `dev.azure.com` / `visualstudio.com` → AzureDevOps (otherwise default AzureDevOps). `Detect` step 4 also delegates to `MapRemoteUrlToProvider` so config-driven and remote-driven URL handling stay in lockstep — earlier the URL→provider rules were duplicated in `InitCommand.DetectProviderFromGitRemote` (now removed) with narrower domain coverage. `GetGitRemoteUrl(string? workingDirectory)` is the shared subprocess helper. |

### CLI infrastructure (REBUSS.Pure\Cli)

| File | Role |
|---|---|
| `REBUSS.Pure\Cli\CliArgumentParser.cs` | Parses CLI args: detects `init` command vs server mode, extracts `--repo`, `--pat`, `--org`, `--project`, `--repository`, `--provider`, `--owner`, `--agent`, `--model` (server mode only — value routed to `CopilotReview:Model` via `CliConfigurationBuilder`, replacing the removed `appsettings.json` entry); parses `--pat`, `-g`/`--global`, `--ide`, and `--agent` for `init` command; `CliParseResult.IsGlobal` flag, `CliParseResult.Ide`, `CliParseResult.Agent`, and `CliParseResult.Model` properties. `--agent` accepts `copilot` or `claude` (case-insensitive); unknown values throw `ArgumentException`. Constants `AgentCopilot`/`AgentClaude` exposed for consumers. |
...
| `REBUSS.Pure\Services\CopilotReview\ClaudeCliAvailabilityDetector.cs` | `ICopilotAvailabilityDetector` registered instead of `CopilotAvailabilityDetector` when `--agent claude`. Never starts the Copilot SDK and does no environment probing: verdict is `Ok` when `CopilotReview:Enabled` is true, otherwise `DisabledByConfig` (so strict mode never throws). Environment problems (`claude` not on PATH, not logged in) surface per page from `ClaudeCliAgentInvoker` as failed page reviews. |
...
| `REBUSS.Pure\Services\CopilotReview\AgentReviewOrchestrator.cs` | Feature 013 + 021 — top-level orchestrator (~200 LOC after the registry/executor/pipeline split). Implements `IAgentReviewOrchestrator` (trigger/wait/snapshot) + `IAsyncDisposable`. Composes three focused collaborators: `AgentReviewJobRegistry` (idempotent registration via `TryRegister`, terminal-state writes via `CompleteUnderLock`, snapshot projection), `PageReviewExecutor` (batched parallel page dispatch + 3-attempt retry), `FindingValidationPipeline` (Feature 021 post-page validation; opt-in via `IsEnabled`). Owns the `IHostApplicationLifetime` cancellation token, the fire-and-forget background task lifetime, and the dispose-time drain (`Task.WhenAll` with 5-second timeout — graceful but bounded). `BackgroundBodyAsync` is now pure orchestration: re-paginate via `IPageAllocator` against `CopilotReview.ReviewBudgetTokens` → empty-fast-path → executor → conditional pipeline (try/catch graceful degradation per FR-012) → publish final `CurrentActivity` → `CompleteUnderLock(Ready)`. OCE/general failure branches `CompleteUnderLock(Failed)` + `TrySetCanceled`/`TrySetException`. Narrow exception to Principle VI. |
| `REBUSS.Pure\Services\CopilotReview\AgentReviewJob.cs` | `internal sealed class AgentReviewJob` — per-review job state. Mutable POCO guarded by the registry's lock for terminal-state writes (`Status`, `Result`, `ErrorMessage`, `CompletedAt`, `TotalPages`) and by `ConcurrentDictionary` for concurrent add/lookup. Per-page progress fields (`CompletedPages`, `CurrentActivity`) use lock-free primitives (`Interlocked` / `volatile`) because parallel page tasks update them without taking the lock. Promoted from a private nested class on `AgentReviewOrchestrator` to a file-level `internal` type so the executor and pipeline collaborators can mutate its state across files. |
| `REBUSS.Pure\Services\CopilotReview\AgentReviewJobRegistry.cs` | `internal sealed class AgentReviewJobRegistry(IOptions<CopilotReviewOptions> options)` — owns the `ConcurrentDictionary<string, AgentReviewJob>` and the `_lock` guarding terminal-state transitions and snapshot reads. `TryRegister(reviewKey)` runs an opportunistic TTL sweep (controlled by `JobRetentionMinutes`; `≤ 0` disables it) then atomically registers a new job or returns `null` for idempotent skip when the key is already in flight. `TryGet`/`All` are dictionary-level reads. `CompleteUnderLock(job, status, result, errorMessage)` and `SetTotalPagesUnderLock(job, totalPages)` perform mutations under the lock so concurrent `Snapshot(reviewKey)` readers never observe torn writes across the multi-field terminal transition. |
| `REBUSS.Pure\Services\CopilotReview\PageReviewExecutor.cs` | `internal sealed class PageReviewExecutor(IAgentPageReviewer, IOptions<CopilotReviewOptions>, ILogger)` — per-page work. `ExecuteAllAsync(job, allocation, enrichment, ct)` returns a slot-aligned `AgentPageReviewResult[]`. Dispatches pages in batches of `Math.Max(1, MaxConcurrentPages)`: within a batch each page runs on its own `Task.Run`; `Task.WhenAll(batchTasks)` joins before the next batch starts. The cap exists because the GitHub Copilot backend silently re-queues larger fan-outs, so an unbounded `Task.WhenAll` over all pages does not actually run faster — the `CopilotRequestThrottle` (3-second `SemaphoreSlim` gate) serializes outgoing SDK calls while model response wait times overlap inside the batch. Each page task writes to its own `pageResults[idx]` slot (no contention). Owns the `MaxAttemptsPerPage = 3` retry loop (immediate retry, no backoff, fills `FailedFilePaths` on exhaustion — the executor is the only component that knows which files were on the page). Owns `BuildPageInput` — concatenates `IEnrichmentResult.EnrichedByPath` entries for the page's items into the SDK-bound payload. Atomically increments `AgentReviewJob.CompletedPages` per page completion via `Interlocked.Increment`. `AgentReviewWaiter` polls this counter for order-agnostic progress notifications (per-page `CurrentActivity` is intentionally **not** published — parallel tasks would overwrite it and cause flip-flop in the IDE feed; the monotonic counter is the sole page-level signal). |
| `REBUSS.Pure\Services\CopilotReview\FindingValidationPipeline.cs` | `internal sealed class FindingValidationPipeline(IOptions<CopilotReviewOptions>, ILogger, FindingValidator?, FindingScopeResolver?)` — Feature 021 post-page validation. `IsEnabled` short-circuits when `ValidateFindings == false` OR either optional dependency is null (DI may not wire them — Constitution Principle V deferred resolution: flag is read at first review). `RunAsync(job, pageResults, ct)` runs 4 phases in place: (1) parse findings per succeeded page tracking page origin, (2) resolve enclosing scopes via `FindingScopeResolver` (truncated when method > `MaxScopeLines`), (3) delegate to `FindingValidator` (severity-ordering + `IPageAllocator`-based pagination) with a `pageProgress` callback that updates `job.CurrentActivity` per validation page, (4) map verdicts back per page and rebuild `ReviewText` via `FindingFilterer`. Skips validation entirely when total findings exceed `MaxValidatableFindings` (Warning log surfaces the bypass). `LogValidationSummary` emits a single Information line so silent drop paths (e.g., archive at the wrong commit producing `SourceUnavailable` for every added file) are visible without inspection-file archaeology. Exceptions propagate; the orchestrator wraps the call in a try/catch for graceful degradation (FR-012). |
| `REBUSS.Pure\Services\CopilotReview\CopilotReviewOptions.cs` | Feature 013 + 018 + 021 — `IOptions<T>` binding for `CopilotReview` section: `Enabled`, `ReviewBudgetTokens` (default 128 000), `Model` (default blank — filled per agent by `CopilotReviewModelDefaults`; consts `DefaultCopilotModel = "gpt-5.4"`, `DefaultClaudeModel = "claude-sonnet-4.6"`, static `DefaultModelFor(agent)`), `GitHubToken` (feature 018 FR-009), `StrictMode` (feature 018 FR-015), `ValidateFindings` (default `true`), `MaxValidatableFindings` (default 40 — over-threshold skips validation entirely), `MaxScopeLines` (default 150 — truncate enclosing method bodies above this), `MaxConcurrentPages` (default 6 — batch size for parallel page dispatch; caps simultaneously in-flight Copilot requests to avoid GitHub's silent overflow re-queueing), `MinRequestIntervalSeconds` (default 3.0 — minimum spacing between outbound Copilot SDK calls, consumed by `CopilotRequestThrottle`), `JobRetentionMinutes` (default 30), `PerPageTimeoutMinutes` (default 5, clamp ≥ 1 — per-page hard timeout for `ClaudeCliAgentInvoker`; exceeded pages raise `TimeoutException` and are retried up to 3× before being surfaced as per-page failure without aborting siblings; no effect on `--agent copilot`), `CopilotCliPath` (nullable; absolute path to a standalone Copilot CLI binary; overrides the SDK's bundled-binary lookup when non-blank), plus the consts `GitHubTokenEnvironmentVariable = "REBUSS_COPILOT_TOKEN"` and `CopilotCliPathEnvironmentVariable = "REBUSS_COPILOT_CLI_PATH"`. |
| `REBUSS.Pure\Services\CopilotReview\CopilotReviewModelDefaults.cs` | `internal sealed class CopilotReviewModelDefaults(string? agent) : IPostConfigureOptions<CopilotReviewOptions>` — fills a blank `Model` with `CopilotReviewOptions.DefaultModelFor(agent)` so `--agent claude` never sends the Copilot default (`gpt-5.4`) to the Claude CLI. Explicit `--model` / config values win. Registered in `ServiceCollectionExtensions` with the parsed `--agent`, and in `CopilotAgentSetupStep`'s init-time probe container with `copilot`. |
| `REBUSS.Pure\Services\CopilotReview\CopilotUnavailableMessage.cs` | Static helper used by `GetPullRequestContentToolHandler` and `GetLocalContentToolHandler` to build the reason-aware `McpException` message when `ICopilotAvailabilityDetector.IsAvailableAsync` returns false. For `CopilotAuthReason.DisabledByConfig` returns `ErrorAgentReviewLayerRequired` (guidance to set `CopilotReview:Enabled = true`); for every other reason formats `ErrorAgentReviewLayerUnavailable` ("AI review layer unavailable ({Reason}). {Remediation}") so the operator sees the actual failure path. Wording is agent-neutral — provider-specific remediation lives in the verdict's `Remediation` field, set by the runner that produced the verdict. |
...
| `REBUSS.Pure\Services\CopilotReview\Validation\FindingScopeResolver.cs` | Feature 021 + Feature 023 — resolves the scope context for each finding and gracefully falls back so that every `.cs` finding reaches Copilot validation. **Feature 023 changed the source-resolution path**: instead of `DiffSourceResolver` (PR-archive only), the resolver now depends on `IFindingSourceProviderSelector` and accepts a `reviewKey` argument on `ResolveAsync` so PR vs. local mode selection happens per call. Chain (per-finding, per-file source resolved once): (1) non-`.cs` → `ScopeResolutionFailure.NotCSharp` (passthrough, no provider call — FR-007); (2) `provider = _selector.SelectFor(reviewKey)` then `await provider.GetAfterCodeAsync(filePath, ct)` — null → `SourceUnavailable`; (3) `.cs` with source → determine effective line: use `LineNumber` as-is when present, else call `FindingLineResolver.TryResolveLine` over the backtick identifiers in the description; (4) call `FindingScopeExtractor.ExtractScopeBody(line, maxScopeLines)` — truncates around the finding line via `TruncateAroundLine` when the method exceeds `MaxScopeLines` (window ±`MaxScopeLines/2` plus `// ... (X lines omitted) ...` markers); (5) if extraction failed, retry step (3)-(4) passing the original line as a hint to `FindingLineResolver`; (6) still failing → whole-file fallback via `BuildWholeFileFallback` (truncate to `MaxScopeLines × 2` keeping head + tail with `// ... omitted from middle ...` marker) and scope name `<entire file: {path}>` — still `ResolutionFailure.None` so it reaches Copilot. |
| `REBUSS.Pure.Core\IFindingSourceProvider.cs` | Feature 023 — narrow contract: `Task<string?> GetAfterCodeAsync(string filePath, CancellationToken ct)`. Returns the file's after-state content for validation, or null when unavailable (missing, oversize, source root unresolvable). Implementations must not throw for routine "not found" cases. Cancellation propagates. |
| `REBUSS.Pure.Core\IFindingSourceProviderSelector.cs` | Feature 023 — picks an `IFindingSourceProvider` per review key. Local prefixes (`local:staged:`, `local:working-tree:`, `local:branch-diff:`) — matching `LocalReviewScope.ToString()` and the form `GetLocalContentToolHandler` actually emits — map to a per-call wrapper bound to the matching git ref; all other review keys map to the singleton remote provider. Returned wrapper is review-scoped — callers must not cache across reviews. |
| `REBUSS.Pure\Services\CopilotReview\Validation\RemoteArchiveSourceProvider.cs` | Feature 023 — PR-mode `IFindingSourceProvider`. Wraps `IRepositoryDownloadOrchestrator.GetExtractedPathAsync` + `RepositoryFileResolver.ResolveRoot/ResolvePath` + 100 KB cap + `File.ReadAllTextAsync`. 3-minute internal timeout matches the prior `DiffSourceResolver` PR-mode behavior so FR-003 (PR no-regression) holds. Caller-`ct` cancellation propagates; internal-timeout cancellation returns null. |
| `REBUSS.Pure\Services\CopilotReview\Validation\LocalWorkspaceSourceProvider.cs` | Feature 023 — local-mode source provider used by the per-call wrapper. Resolves workspace root **per call** (`IWorkspaceRootProvider.ResolveRepositoryRoot()`, Constitution §V), then delegates to `ILocalGitClient.GetFileContentAtRefAsync(root, filePath, gitRef, ct)`. Enforces 100 KB cap (parity with PR mode, FR-005). Invokes `onWorkspaceRootMissing` callback once per missing-root call so the wrapper can dedupe to a single review-level warning (FR-006). `OperationCanceledException` propagates; other exceptions are logged Debug → null. Does NOT itself implement `IFindingSourceProvider` — the selector wraps it. |
| `REBUSS.Pure\Services\CopilotReview\Validation\FindingSourceProviderSelector.cs` | Feature 023 — singleton `IFindingSourceProviderSelector` impl. Maps review-key prefix to source: `local:staged:` → `LocalGitClient.IndexRef`, `local:working-tree:` → `LocalGitClient.WorkingTreeRef`, `local:branch-diff:` → `"HEAD"`, default → singleton `RemoteArchiveSourceProvider`. Prefixes mirror `LocalReviewScope.ToString()` exactly — any divergence from those strings would silently route local reviews to the remote archive provider and degrade every finding to "uncertain". For local prefixes returns a per-call private `BoundLocalSourceProvider` wrapper carrying the bound git ref + a `_warnedRootMissing` boolean for FR-006 dedup. The wrapper is a value object (not DI-registered), so its mutable field is review-scoped, not service-scoped — Constitution §VI is preserved. |
...
| `REBUSS.Pure\Cli\InitCommand.cs` | Steps 2–8 refactor — top-level orchestrator (~150 LOC) for the `init` command. User inputs are carried as a single immutable options DTO `InitCommandOptions` (`_options` field). Three public/internal constructors are thin shims that build the record and forward to the canonical `(TextWriter, TextReader, InitCommandOptions, processRunner, localConfigStore?, gitHubConfigStore?, globalConfigTargetsResolver?)` constructor. `ExecuteAsync` now reads as straight orchestration: find git root → resolve agent (via `PromptForAgentAsync` when `--agent` not provided) → resolve targets via `McpConfigTargetResolver.Resolve` / `ResolveGlobal` → write configs via `McpConfigWriter` → deploy prompts via `PromptDeployer` → deploy skills via `ClaudeSkillDeployer` → back up legacy commands via `LegacyClaudeCommandBackup` → clear provider caches → run auth flow via `CreateAuthFlow` (delegates SCM provider detection to `ProviderDetector.DetectFromGitRemote(_options.WorkingDirectory)` per Step 1 consolidation) → run agent setup via `AgentSetupStepFactory.Create(...).RunAsync(...)` → emit trailing hints. Locally retained helpers: `PromptForAgentAsync`, `CreateAuthFlow`, `FindGitRepositoryRoot`, plus the `RunProcessAsync` / `RunInteractiveProcessAsync` static subprocess helpers (Step 9 will pull these into `ICliProcessRunner`). |
| `REBUSS.Pure\Cli\InitCommandOptions.cs` | Step 2 refactor — `internal sealed record InitCommandOptions(string WorkingDirectory, string ExecutablePath, string? Pat, bool IsGlobal, string? Ide, string? Agent, string? DetectedProvider)`. Single immutable carrier for the user-facing inputs to `init`. Collaborator/test seams (`TextWriter`/`TextReader`, `processRunner`, `ILocalConfigStore?`, `IGitHubConfigStore?`, `Func<List<McpConfigTarget>>?`) intentionally live alongside the record on the canonical `InitCommand` constructor — they are not part of the options record. Used to thread user choices to subsequent extracted collaborators (resolver, writer, deployers, agent setup steps in later refactor steps). |
| `REBUSS.Pure\Cli\Mcp\McpConfigJsonBuilder.cs` | Step 3 refactor — `internal static class McpConfigJsonBuilder` with two entry points: `Build(string normalizedExePath, string normalizedRepoPath, string? pat = null, bool useMcpServersKey = false, string? agent = null, string? model = null)` emits a fresh document via string interpolation of pre-escaped paths; `Merge(string existingJson, string rawExePath, string rawRepoPath, string? pat = null, bool useMcpServersKey = false, string? agent = null, string? model = null)` upserts the `REBUSS.Pure` server entry into existing JSON via `Utf8JsonWriter` (raw paths in, escaping done by writer), preserves all unrelated top-level keys + other server entries, and carries over an existing `--pat` and `--model` arg when the caller passes null/whitespace for the corresponding parameter. On invalid JSON falls back to `Build`. Private generic `ExtractExistingArg(JsonElement, string, string argName)` reads the carried-over value for both (flag match is case-insensitive, mirroring `CliArgumentParser`). Wire format unchanged from the prior `InitCommand.BuildConfigContent` / `MergeConfigContent` — same key order, same arg order (`--repo`, value, optional `--pat`, optional `--agent`, optional `--model`). |
| `REBUSS.Pure\Cli\Mcp\McpConfigTargetResolver.cs` | Step 4 refactor — `internal static class McpConfigTargetResolver`. Two entry points: `Resolve(string gitRoot, string? ide = null, string? agent = null)` for repository-local targets (Claude Code single-file mode when `agent="claude"`; explicit `--ide vscode`/`vs` short-circuits auto-detection; otherwise IDE folder existence drives the result — both written when neither marker is present); `ResolveGlobal(string? agent = null)` for user-level targets (single `~/.claude.json` for Claude vs. VS+VS Code+Copilot CLI trio for Copilot/null). Also owns the IDE-detection helpers (`DetectsVsCode`, `DetectsVisualStudio`, `DetectsClaudeCode`) and the directory/filename constants (`VsCodeDir`, `VisualStudioDir`, `ClaudeCodeDir`, `ClaudeCodeMarkerFile`, `McpConfigFileName`, `VsGlobalMcpConfigFileName`, `CopilotCliMcpConfigFileName`, `ClaudeCodeMcpConfigFileName`, `ClaudeCodeGlobalConfigFileName`) — kept `internal` so subsequent refactor steps (Step 5 deployers) reference them through this resolver instead of redefining them. Bogus `--ide` value throws `ArgumentException` with the supported-values hint. |
| `REBUSS.Pure\Cli\Mcp\McpConfigTarget.cs` | Step 4 refactor — `internal sealed record McpConfigTarget(string IdeName, string Directory, string ConfigPath, bool UseMcpServersKey = false)`. Moved out of `InitCommand.cs`. Namespace intentionally kept as `REBUSS.Pure.Cli` (not `REBUSS.Pure.Cli.Mcp`) so existing test files and the `Func<List<McpConfigTarget>>` global-targets resolver seam continue to compile without renaming usings. |
| `REBUSS.Pure\Cli\Deployment\EmbeddedResourceLocator.cs` | Step 5 refactor — `internal static class EmbeddedResourceLocator` with single entry point `Find(Assembly, string prefix, string fileName)`. Extracted verbatim from `InitCommand.FindResourceName`: tries the exact name first, then the hyphen→underscore mangled fallback, returns null if neither hits. Shared between `PromptDeployer` and `ClaudeSkillDeployer` so a future addition under either prefix automatically inherits the same defensive resolution. |
| `REBUSS.Pure\Cli\Deployment\PromptDeployer.cs` | Step 5 refactor — `internal sealed class PromptDeployer(TextWriter output, Assembly? assembly = null)`. Owns `PromptFileNames` / `LegacyPromptFileNames` / `ResourcePrefix` constants. `DeployAsync(string gitRoot, CancellationToken ct)` writes `.github/prompts/<name>.prompt.md` for every entry in `PromptFileNames`, deletes any legacy non-suffixed copies (`<name>.md`), emits the `MsgCopiedPrompts` count log, overwrites on repeat runs. Always runs regardless of `--agent` (Feature 024 D4). Optional Assembly parameter exists for testability. |
| `REBUSS.Pure\Cli\Deployment\ClaudeSkillDeployer.cs` | Step 5 refactor — `internal sealed class ClaudeSkillDeployer(TextWriter output, Assembly? assembly = null)`. Owns `SkillNames` (also consumed by `LegacyClaudeCommandBackup`) and `SkillsResourcePrefix`. `DeployAsync(string gitRoot, CancellationToken ct)` writes `.claude/skills/<name>/SKILL.md` for every skill name. Drift policy: identical content is a no-op (`LogInitSkillUnchanged`); different content backs up the user version to `SKILL.md.bak` before overwriting. Idempotent on identical content. |
| `REBUSS.Pure\Cli\Deployment\LegacyClaudeCommandBackup.cs` | Step 5 refactor — `internal sealed class LegacyClaudeCommandBackup(TextWriter output)`. `RunAsync(string gitRoot, CancellationToken ct)` enumerates `ClaudeSkillDeployer.SkillNames` and moves any pre-024 `.claude/commands/<skill>.md` to `<skill>.md.bak`. Unrelated user files in `commands/` left alone; missing directory is a no-op. |
| `REBUSS.Pure\Cli\Mcp\McpConfigWriter.cs` | Step 6 refactor — `internal sealed class McpConfigWriter(TextWriter output)`. `WriteAllAsync(IEnumerable<McpConfigTarget> targets, string executablePath, string gitRoot, string? pat, string? agent, CancellationToken ct)` owns the per-target IO loop: `Directory.CreateDirectory(target.Directory)`, read existing → `McpConfigJsonBuilder.Merge` (raw paths) or fresh build → `McpConfigJsonBuilder.Build` (normalized paths), `.bak` on overwrite (catches `IOException` / `UnauthorizedAccessException` only — `OperationCanceledException` propagates so Ctrl+C is not masked), write file (catches `IOException` and emits `Resources.ErrMcpConfigLocked` then continues with the next target rather than aborting the whole init), success log (`MsgUpdated…` / `MsgCreated…`). Path normalization (`"\\" → "\\\\"`) lives inside the writer so this class is the sole owner of the "raw vs. escaped path" concern. |
| `REBUSS.Pure\Cli\AgentSetup\IAgentSetupStep.cs` | Step 7 refactor — polymorphic seam: `internal interface IAgentSetupStep { Task RunAsync(CancellationToken ct); }`. Implementations preserve the same soft-exit invariant the previous private `RunCopilotSetupStepAsync` / `RunClaudeSetupStepAsync` had — failures never affect init's exit code (FR-011). |
| `REBUSS.Pure\Cli\AgentSetup\CopilotAgentSetupStep.cs` | Step 7 refactor — `internal sealed class CopilotAgentSetupStep` implements `IAgentSetupStep`. Constructor takes `(TextWriter, TextReader, processRunner, string? ghCliPathOverride)`. `RunAsync` builds the narrow throwaway DI container (private static `BuildCopilotProbeServices` — extracted verbatim from `InitCommand.BuildCopilotProbeServices`) that registers only the types needed by `ICopilotVerificationProbe`, then runs `CopilotCliSetupStep`. Probe construction failure logs a warning and continues without verification; setup-step failure is swallowed; provider is disposed in a `finally`. |
| `REBUSS.Pure\Cli\AgentSetup\ClaudeAgentSetupStep.cs` | Step 7 refactor — `internal sealed class ClaudeAgentSetupStep` implements `IAgentSetupStep`. Constructor takes `(TextWriter, TextReader, processRunner)`. `RunAsync` adapts the 2-arg `processRunner` into the 3-arg `(exe, args, ct)` form `ClaudeCliSetupStep` uses (concatenating exe + args verbatim — discarding exe would break probe disambiguation across `claude`/`winget`/`npm`), constructs a `ClaudeVerificationRunner` probe, and runs `ClaudeCliSetupStep`. Any failure is soft-exited. |
| `REBUSS.Pure\Cli\AgentSetup\AgentSetupStepFactory.cs` | Step 7 refactor — `internal sealed class AgentSetupStepFactory(TextWriter, TextReader, processRunner)`. `Create(string? agent, string? ghCliPathOverride)` returns `ClaudeAgentSetupStep` for `CliArgumentParser.AgentClaude` (case-insensitive) and `CopilotAgentSetupStep` for everything else (`AgentCopilot`, unknown values, null — Copilot is the safe default). Replaces the previous `if/else` block in `InitCommand.ExecuteAsync`. |
| `REBUSS.Pure\Cli\ICliProcessRunner.cs` | Step 9 refactor — `internal interface ICliProcessRunner` with two methods: `RunAsync(string fileName, string args, CancellationToken ct)` returns `(int ExitCode, string StdOut, string StdErr)` for probes/version checks, and `RunInteractiveAsync(string fileName, string args, CancellationToken ct, IDictionary<string, string>? envOverrides = null)` for interactive flows like `az login` / `gh auth login --web` / `claude /login` (inherits parent stdin/stdout/stderr, returns only the exit code). |
| `REBUSS.Pure\Cli\CliProcessRunner.cs` | Step 9 refactor — `internal sealed class CliProcessRunner : ICliProcessRunner`. Default impl with bodies lifted verbatim from the previous `InitCommand.RunProcessAsync` / `InitCommand.RunInteractiveProcessAsync` statics — exception handling, exit-code mapping (`-1` on `Process.Start` returning null), and environment-override merging are bit-exact. Exposes a process-lifetime `Shared` singleton; the runner is stateless so reusing the instance avoids per-call allocation in hot probe loops. Production callers across `Cli\` (`AzureDevOpsCliAuthFlow`, `GitHubCliAuthFlow`, `ClaudeCliSetupStep`, `CopilotCliSetupStep`) reach in via `CliProcessRunner.Shared.RunAsync` / `RunInteractiveAsync`; test seams stay unchanged because each caller already has a `processRunner` `Func` constructor parameter that overrides this default path. |
...
| `REBUSS.Pure\Services\ClaudeCode\IClaudeVerificationProbe.cs` + `ClaudeVerificationRunner.cs` + `ClaudeVerdict.cs` | Diagnostic probe used exclusively by `ClaudeCliSetupStep` to confirm Claude Code is installed AND authenticated. Runs `claude -p "ping" --output-format json` with a 30s timeout, appending `--bare` only when `ANTHROPIC_API_KEY` is set (mirrors `ClaudeCliAgentInvoker.ShouldUseBareMode` so probe and runtime exercise the same auth path). `ClaudeVerdict.Reason` is one of `ok` / `not-installed` / `not-authenticated` (detected via stdout+stderr keyword match: "not logged in", "/login", "unauthenticated", "authentication") / `timeout` / `invalid-response` (exit 0 but JSON does not contain a string `result` field) / `error`. `Remediation` for `not-authenticated` is environment-aware: when API key is set it points the operator at the API console; otherwise it suggests interactive `/login` or `CLAUDE_CODE_OAUTH_TOKEN`. Never throws — all failure modes are encoded in the verdict. Constructor takes optional `environmentLookup` for testability. |
...
| `REBUSS.Pure\Services\AgentInvocation\CopilotAgentInvoker.cs` | `IAgentInvoker` implementation wrapping `ICopilotSessionFactory`. Creates a fresh session per call, subscribes to `AssistantMessageEvent` / `SessionIdleEvent` / `SessionErrorEvent`, sends the prompt, returns the accumulated response text, disposes the session. Owns all SDK side effects (throttling via `CopilotRequestThrottle`, auth startup via `ICopilotClientProvider.TryEnsureStartedAsync`, session deletion on Dispose). Consumed by `AgentPageReviewer` and `FindingValidator`. Honors the `IAgentInvoker` contract that `model` is an optional hint: null/whitespace falls back to `CopilotReviewOptions.Model` (injected via `IOptions<CopilotReviewOptions>`); only an empty fallback (operator misconfig) raises `InvalidOperationException`. |
| `REBUSS.Pure\Services\AgentInvocation\ClaudeCliAgentInvoker.cs` | `IAgentInvoker` implementation that shells out to `claude -p --output-format json` (writes prompt via stdin to avoid command-line length caps). Parses the `result` JSON field; falls back to raw stdout if the wrapper shape is unexpected. **Per-page timeout** (configurable via `CopilotReviewOptions.PerPageTimeoutMinutes`, default 5 min, clamp ≥ 1; resolved via `ResolvePerPageTimeout(explicitOverride, options)` at call time) is enforced through a linked CTS — when the linked CTS fires while the caller's token is *not* canceled, the catch-when clause raises `TimeoutException` (not `OperationCanceledException`). This classification is load-bearing: `AgentPageReviewer` re-throws OCE but converts other exceptions into a failed `AgentPageReviewResult`, and `AgentReviewOrchestrator.ReviewPageWithRetryAsync` retries that failure up to 3 times before surfacing it as a per-page entry — so a single slow page no longer aborts every sibling page in flight. Genuine external cancellation still throws OCE so host shutdown / orchestrator-level cancel teardown still works. The `model` parameter is normalized via `NormalizeModelForClaudeCli` (dot→hyphen, idempotent) before being forwarded as `--model <value>` when non-empty; this bridges the shared `CopilotReviewOptions.Model` value (Copilot uses dotted ids like `claude-sonnet-4.6`) onto the hyphenated form Claude requires (`claude-sonnet-4-6`) so the same config string works under both `--agent` selections. Otherwise Claude uses whichever model the session is logged into. **Auth-mode selection** (via `ShouldUseBareMode(envLookup)`): if `ANTHROPIC_API_KEY` is set, the call adds `--bare` so hooks/skills/MCP servers in `~/.claude` are NOT loaded — pure one-shot inference channel that bills the API console account; otherwise `--bare` is omitted so the CLI uses the user's subscription credential (persistent OAuth from `claude /login` or `CLAUDE_CODE_OAUTH_TOKEN`). Bare mode refuses to read OAuth credentials, so the conditional is required to keep subscription users working without a separate API billing channel. Failure messages include both stdout AND stderr (truncated to 500 chars each) — claude-cli reports auth/quota errors in the JSON payload on stdout, so logging only stderr was a silent-failure trap. Public ctor takes optional `IOptions<CopilotReviewOptions>` (read at call time, Principle V) plus optional `claudeCliPathOverride` and `environmentLookup` for testability; an `internal` ctor accepting a `TimeSpan` per-page-timeout override is used by integration tests that exercise the timeout-vs-cancel classification on hanging child processes. |
| `REBUSS.Pure\Cli\Prompts\review-pr.prompt.md` | Embedded resource: PR review prompt template for Copilot/IDE (Copilot Chat picks them up from `.github/prompts/`). Agent-neutral wording — title "AI-Assisted", references the configured agent generically. Redesigned for `get_pr_metadata`(with budget) + `get_pr_content` page loop workflow. Pattern-matches the `[review-mode: <agent>-assisted]` header to identify the actual backend (copilot/claude). Includes strict "No Repository Exploration" constraint forbidding non-MCP access. **Important**: prompt instructs the agent to surface server-returned remediation verbatim and to NEVER invent setup commands like `gh copilot setup` or `claude /login` unless the error response explicitly mentions them — this prevents Copilot/Claude lingo bleed-through. Always deployed to `.github/prompts/review-pr.prompt.md` regardless of `--agent`. |
| `REBUSS.Pure\Cli\Prompts\self-review.prompt.md` | Embedded resource: self-review prompt template for Copilot/IDE (mirrors `review-pr.prompt.md`'s wording rules); redesigned for `get_local_content` page loop workflow. Always deployed to `.github/prompts/self-review.prompt.md` regardless of `--agent`. |
| `REBUSS.Pure\Cli\Skills\review-pr\SKILL.md` | Feature 024 — Claude Code skill (frontmatter `name: review-pr`, `description: …`, `argument-hint: <PR-number>`). Body is identical to `Cli\Prompts\review-pr.prompt.md` content; the `InitCommandTests.ExecuteAsync_SkillBodyMatchesPromptBody_AfterStrippingFrontmatter` test enforces this. Deployed to `.claude/skills/review-pr/SKILL.md`. Embedded with explicit `LogicalName="REBUSS.Pure.Cli.Skills.review-pr.SKILL.md"` because the directory name's hyphen confuses the default resource-path resolver. |
| `REBUSS.Pure\Cli\Skills\self-review\SKILL.md` | Feature 024 — Claude Code skill for self-review (`description` covers staged/unstaged/branch self-review triggers; `argument-hint: [scope]` with default `staged`). Body identical to `Cli\Prompts\self-review.prompt.md`. Deployed to `.claude/skills/self-review/SKILL.md`. |
| `REBUSS.Pure.Core\Services\AgentInvocation\AgentIdentity.cs` | Wraps the resolved agent name (`"copilot"` / `"claude"`) so tool handlers can label review responses with the actual backend. Registered as a singleton in `Program.cs` from the same `--agent` flag that selects the `IAgentInvoker`. Consumed by `GetLocalContentToolHandler` and `GetPullRequestContentToolHandler` and forwarded to `PlainTextFormatter.FormatAgentReviewHeader`. Replaces the previously hardcoded `[review-mode: copilot-assisted]` marker so a Claude review never reports as Copilot. |
...
| `REBUSS.Pure.AzureDevOps.Tests\Providers\AzureDevOpsDiffProviderTests.cs` | `AzureDevOpsDiffProvider` (post-decomposition integration tests) — full diff/file diff, skip behavior via the orchestrator surface, `IsFullFileRewrite` cases (now calling `FullFileRewriteDetector` directly), ZIP-fallback heuristic (under/at/above threshold paths, hunk correctness when reading from extracted archives, added-file handling when missing from base archive, threshold=0 disables ZIP path, temp directory cleanup); `TryResolveFilePath` cases (now calling `ExtractedArchiveWorkspace` directly). ZIP test helpers write real archives via `System.IO.Compression.ZipArchive`. The 6 `GetSkipReason_*` cases were relocated to `DiffSkipPolicyTests.cs` after the policy was extracted from the orchestrator. |
| `REBUSS.Pure.AzureDevOps.Tests\Providers\Diff\DiffSkipPolicyTests.cs` | `DiffSkipPolicy` — focused unit tests for the extracted skip-reason policy. 6 cases: `delete` change type → `file deleted`; `rename` → `file renamed`; binary extension → `binary file`; generated path → `generated file`; normal source file → null; `add` change type → null. Cases relocated 1:1 from `AzureDevOpsDiffProviderTests` (Step 2 refactor) — same scenarios, narrower seam. |
...
| `REBUSS.Pure.Tests\Services\LocalReview\LocalGitClientParseTests.cs` | `LocalGitClient` porcelain/name-status parsing, `BuildStatusArgs` HEAD/no-HEAD command selection, and `BuildShowArgs` (Feature 023) — `IndexRef` produces `show :<path>`, `WorkingTreeRef` is short-circuited before this helper, arbitrary refs produce `show <ref>:<path>`. All via reflection on internal static methods. |
...
| `REBUSS.Pure.Tests\Cli\CliArgumentParserTests.cs` | `CliArgumentParser` — server mode, `--repo`, `--pat`, `--org`, `--project`, `--repository`, `init` command, `-g`/`--global` flag, `--ide` option, `--agent copilot`/`--agent claude` option (case-insensitive, unknown values throw, present in server mode too), server-mode `--model` (case-insensitive flag, trailing flag ignored), combined args, edge cases |
| `REBUSS.Pure.Tests\Cli\CliConfigurationBuilderTests.cs` | `CliConfigurationBuilder.BuildOverrides` — `--model` maps to `CopilotReview:Model`; absent flag emits no key |
| `REBUSS.Pure.Tests\Services\CopilotReview\CopilotReviewModelDefaultsTests.cs` | `CopilotReviewModelDefaults` — blank model resolves to per-agent default (Copilot → `gpt-5.4`, Claude → `claude-sonnet-4.6`), explicit model wins, blank string falls back |
| `REBUSS.Pure.Tests\ProviderDetection\ProviderDetectorTests.cs` | Step 1 characterization tests for `ProviderDetector.DetectFromGitRemote` / `MapRemoteUrlToProvider` (internal, exposed via `InternalsVisibleTo`). Pins URL→provider mapping: `github.com` HTTPS / SSH / `.git`-suffixed URLs → `GitHub`; `dev.azure.com` / `visualstudio.com` URLs → `AzureDevOps`; null / empty / unknown domains (`bitbucket.org`, `gitlab.com`) → default `AzureDevOps`; non-git working directory → default `AzureDevOps`. 11 tests; passing alongside the 99-test `InitCommandTests` baseline confirms the Step-1 consolidation is behavior-preserving for the InitCommand auth-flow path. |
| `REBUSS.Pure.Tests\Cli\Mcp\McpConfigJsonBuilderTests.cs` | Step 3 refactor — focused unit tests for `McpConfigJsonBuilder.Build` and `Merge`. 8 cases: build emits `servers` key without PAT/agent; build with PAT + agent inserts both args; `useMcpServersKey:true` switches to `mcpServers` top-level key; merge preserves an unrelated server entry; merge carries over existing `--pat` when caller passes null; caller's PAT wins when both are present; invalid JSON falls back to the build shape; blank/whitespace agent is omitted while non-blank agent is inserted. Defense in depth alongside the existing `InitCommandTests` regression cases — those still call `McpConfigJsonBuilder.Build/Merge` directly post-rename. |
| `REBUSS.Pure.Tests\Cli\Mcp\McpConfigTargetResolverTests.cs` | Step 4 refactor — focused unit tests for `McpConfigTargetResolver.Resolve` and `ResolveGlobal`. 9 cases: `agent="claude"` + local → single Claude Code target with `mcpServers` key; `agent="claude"` + global → single `~/.claude.json` target; `--ide vscode` → single VS Code target; `--ide vs` → single VS target; bogus `--ide` → `ArgumentException`; auto-detect with only `.vscode/` → VS Code only; auto-detect with only a `.sln` at root → VS only; auto-detect with neither marker → both targets; `ResolveGlobal(null)` → 3 Copilot targets (VS global + VS Code global + Copilot CLI global). Uses a small disposable `TempRepoDirectory` helper for cases that need a real temp directory. |
| `REBUSS.Pure.Tests\Cli\Deployment\EmbeddedResourceLocatorTests.cs` | Step 5 refactor — focused unit tests for `EmbeddedResourceLocator.Find`. 3 cases: exact match found via Copilot prompt resource; hyphen-pinned LogicalName resource (Claude skill) resolves via exact match; entirely missing prefix returns null. Uses the production REBUSS.Pure assembly so we exercise real manifest names. |
| `REBUSS.Pure.Tests\Cli\Deployment\PromptDeployerTests.cs` | Step 5 refactor — focused unit tests for `PromptDeployer.DeployAsync`. 3 cases: fresh deploy writes `review-pr.prompt.md` and `self-review.prompt.md` and emits the count log; legacy non-suffixed prompt file (`review-pr.md`) is deleted on deploy; repeated deploy overwrites a user-modified prompt (deploy is the source of truth). Uses disposable temp dirs. |
| `REBUSS.Pure.Tests\Cli\Deployment\ClaudeSkillDeployerTests.cs` | Step 5 refactor — focused unit tests for `ClaudeSkillDeployer.DeployAsync`. 3 cases: fresh target writes `.claude/skills/<name>/SKILL.md` for every skill; identical-content second run is a no-op (no `.bak`, "unchanged" log); user-edited content triggers a `.bak` backup before overwriting. Uses disposable temp dirs. |
| `REBUSS.Pure.Tests\Cli\Deployment\LegacyClaudeCommandBackupTests.cs` | Step 5 refactor — focused unit tests for `LegacyClaudeCommandBackup.RunAsync`. 3 cases: pre-existing `.claude/commands/review-pr.md` is moved to `.bak`; unrelated `my-custom.md` user file is left alone; missing `.claude/commands/` directory is a silent no-op. Uses disposable temp dirs. |
| `REBUSS.Pure.Tests\Cli\Mcp\McpConfigWriterTests.cs` | Step 6 refactor — focused unit tests for `McpConfigWriter.WriteAllAsync`. 3 cases: new path creates the directory, writes the file, no `.bak`, "Created" log; existing valid JSON is backed up, content is merged (other server entries preserved), "Updated" log; multiple targets each written independently with the same PAT/agent forwarded. File-locked-by-running-client behaviour is exercised through the integration tests in `InitCommandTests` rather than reproduced here. |
| `REBUSS.Pure.Tests\Cli\AgentSetup\AgentSetupStepFactoryTests.cs` | Step 7 refactor — focused dispatch tests for `AgentSetupStepFactory.Create`. 5 cases: `agent="claude"` → `ClaudeAgentSetupStep`; case-insensitive `"CLAUDE"` → `ClaudeAgentSetupStep`; `agent="copilot"` → `CopilotAgentSetupStep`; null agent defaults to `CopilotAgentSetupStep`; unknown agent value also defaults to `CopilotAgentSetupStep`. Setup-step run-time behaviour is covered by the existing `CopilotCliSetupStepTests` / `ClaudeCliSetupStepTests` integration suites — this class only exercises the dispatch contract. |
| `REBUSS.Pure.Tests\Cli\InitCommandTests.cs` | `InitCommand` — generates `mcp.json` (local and global `-g` mode), `--ide vscode`/`--ide vs` explicit IDE targeting, **agent selection** (`PromptForAgentAsync` empty/`1`/`copilot` → copilot, `2`/`claude`/`claude-code` → claude, case-insensitive, unknown → default copilot; `--agent` flag skips the prompt), **Claude Code config targets** (single `.mcp.json` with `mcpServers` key, no VS/VS Code output, backup `.bak` before overwrite, merge preserves unknown top-level keys, global mode writes only `~/.claude.json`), **agent arg in mcp.json** (`"--agent", "copilot"`/`"--agent", "claude"` added to generated args), **Feature 024 — skills replace slash commands**: theory `[InlineData("claude")] [InlineData("copilot")]` confirms both `.github/prompts/<name>.prompt.md` and `.claude/skills/<name>/SKILL.md` are created regardless of agent, frontmatter present in skills, no `.claude/commands/` written. Legacy cleanup test: pre-existing `.claude/commands/<skill-name>.md` is moved to `.bak`; unrelated user commands in `.claude/commands/` left alone. Desync guard: skill body (after stripping frontmatter) matches prompt body verbatim. Idempotency: second `init` produces no `.bak` and emits "unchanged" log. Drift handling: user-modified `SKILL.md` is backed up to `.bak` before re-deploying embedded source. Plus copies prompt and instruction files (always overwrites), error cases, subdirectory support, Azure DevOps CLI login, GitHub CLI login, PAT carry-over; global-mode tests use nested `GlobalTestContext` disposable fixture (creates temp Git repo + simulated global config dirs), inject `globalConfigTargetsResolver` pointing to temp dirs so no real user-home files are touched |
...
| `REBUSS.Pure.Tests\Services\AgentInvocation\ClaudeCliAgentInvokerTests.cs` | `ClaudeCliAgentInvoker.ExtractResultFromJson` (internal) — valid JSON with `result` string → value; empty/whitespace → empty; missing `result` → raw stdout; invalid JSON → raw stdout; non-string `result` → raw stdout. `ShouldUseBareMode` — true only when `ANTHROPIC_API_KEY` is set (blank/missing → false; OAuth-only env → false). `FormatFailureDetail` — empty/empty → "no output captured."; stdout-only path included (regression guard for the silent-failure trap when claude-cli reports auth errors on stdout); both present → both labelled. `NormalizeModelForClaudeCli` — theory mapping `claude-sonnet-4.6` → `claude-sonnet-4-6`, `claude-opus-4.7` → `claude-opus-4-7`, `claude-haiku-4.5-20251001` → `claude-haiku-4-5-20251001`, plus idempotency for already-canonical ids and aliases (`sonnet`); regression guard for the HTTP 404 from `claude -p --model` when the dotted Copilot id was forwarded verbatim. End-to-end subprocess invocation is not unit-tested — exercised only when a real `claude` binary is available. |
...
| `REBUSS.Pure.Tests\Services\CopilotReview\ClaudeCliAvailabilityDetectorTests.cs` | `ClaudeCliAvailabilityDetector`: enabled -> `Ok` verdict with configured model, `DisabledByConfig` in graceful and strict mode without throwing, cancelled token propagates. |
...
| `REBUSS.Pure.Tests\Services\CopilotReview\AgentReviewOrchestratorTests.cs` | `AgentReviewOrchestrator` (post-decomposition integration tests) — trigger/wait/snapshot lifecycle, parallel page dispatch via mocked `IAgentPageReviewer`, retry-loop exhaustion with `FailedFilePaths`, empty-allocation fast path, validation opt-out path (null `FindingValidator`/`FindingScopeResolver` → `FindingValidationPipeline.IsEnabled == false`), TTL retention sweep + retention-disabled paths via reflection on the registry. 19 cases — the regression contract for the orchestrator-level pipeline. |
| `REBUSS.Pure.Tests\Services\CopilotReview\AgentReviewJobRegistryTests.cs` | `AgentReviewJobRegistry` — focused unit tests for the registry collaborator. 7 cases: fresh-key registers + stores; same-key second call returns null (idempotent); after-retention-expired evicts the terminal job and registers a fresh one; retention disabled (`JobRetentionMinutes = 0`) preserves the existing job and stays idempotent even with a 30-day-old `CompletedAt`; `Snapshot` after `CompleteUnderLock` reflects new status + progress fields (`TotalPages`/`CompletedPages`/`CurrentActivity`); unknown-key snapshot returns null; `All` exposes all live registrations. |
| `REBUSS.Pure.Tests\Services\CopilotReview\PageReviewExecutorTests.cs` | `PageReviewExecutor` — focused unit tests for the per-page collaborator. 7 cases: all-pages-succeed first attempt + AttemptsMade=1; transient-failure-then-success → AttemptsMade=2; all-attempts-exhausted → Failure with FailedFilePaths populated by the executor + AttemptsMade=3; `MaxConcurrentPages=2` with 5 pages → 3 batches, slot integrity preserved; empty allocation → empty array, zero SDK calls; reviewer throws → treated as failed attempt + retried up to 3 times; pre-cancelled token propagates `OperationCanceledException`. |
| `REBUSS.Pure.Tests\Services\CopilotReview\FindingValidationPipelineTests.cs` | `FindingValidationPipeline` — focused unit tests for the predicate + disabled-mode no-op. 4 cases: flag off → `IsEnabled = false`; flag on but validator/resolver null → still false; `RunAsync` when disabled leaves `pageResults` unchanged (no mutation of `ReviewText`); `RunAsync` when disabled does not touch `job.CurrentActivity`. The full 4-phase happy path is exercised via production DI in the integration tests under `AgentReviewOrchestratorTests` — the validator and scope-resolver collaborators are `sealed` and pull in heavyweight sub-dependencies (`IAgentInvoker`, `ITokenEstimator`, `IFindingSourceProviderSelector`) that are not worth standing up in pure unit tests. |
...
| `REBUSS.Pure.Tests\Services\CopilotReview\Validation\FindingScopeResolverTests.cs` | `FindingScopeResolver` (Feature 021 + Feature 023) — non-`.cs` passthrough (asserts provider is NOT called, FR-007), missing source, truncation around the finding line when method exceeds `MaxScopeLines`, smart-line recovery (backtick identifier in description when `LineNumber` is null), hint-retry (line on a `using` → retry with identifier lookup), whole-file fallback when nothing matches (still `ResolutionFailure.None` with scope name `<entire file: path>`), reviewKey forwarding to selector (`PassesReviewKeyToSelector`). All tests use a mocked `IFindingSourceProviderSelector` returning a stub `IFindingSourceProvider`. |
| `REBUSS.Pure.Tests\Services\CopilotReview\Validation\RemoteArchiveSourceProviderTests.cs` | Feature 023 — happy path against a temp extracted archive, `GetExtractedPathAsync` returns null, file missing, file > 100 KB → null, caller cancellation propagates, internal-timeout cancellation returns null (FR-003 PR-mode parity), generic orchestrator exception bubbles up. |
| `REBUSS.Pure.Tests\Services\CopilotReview\Validation\LocalWorkspaceSourceProviderTests.cs` | Feature 023 — happy path for each of the three gitRefs (`IndexRef`, `WorkingTreeRef`, `"HEAD"`), workspace root null/empty invokes the callback exactly once and returns null without calling `ILocalGitClient`, file not present at ref → null, content > 100 KB → null, `OperationCanceledException` propagates, generic git exception logged Debug → null, `ResolveRepositoryRoot()` is called per invocation (Constitution §V). |
| `REBUSS.Pure.Tests\Services\CopilotReview\Validation\FindingSourceProviderSelectorTests.cs` | Feature 023 — selector mapping per prefix: `local:staged:` → `IndexRef`, `local:working-tree:` → `WorkingTreeRef`, `local:branch-diff:` → `"HEAD"`. Theory covering non-local prefixes (PR-style keys, empty key, bare `local:`, case-mismatched `LOCAL:STAGED:`) returns the singleton `RemoteArchiveSourceProvider`. Per-wrapper warning de-duplication (FR-006) verified by counting `LogWarning` calls across multiple `GetAfterCodeAsync` invocations within one wrapper, and by checking that two distinct `SelectFor` calls each warn once. |
| `REBUSS.Pure.Tests\Services\CopilotReview\Validation\FindingValidationWiringTests.cs` | Feature 023 — integration-style wiring tests. Real `FindingScopeResolver` + real `FindingSourceProviderSelector` + real provider impls, only externals (`IRepositoryDownloadOrchestrator`, `IWorkspaceRootProvider`, `ILocalGitClient`) mocked. Verifies each user-story review mode reaches the correct primitive: `local:staged:` invokes `ILocalGitClient.GetFileContentAtRefAsync(..., IndexRef, ...)` and does NOT touch the orchestrator (US1); `local:working-tree:` → `WorkingTreeRef` (US2); `local:branch-diff:<base>:` → `"HEAD"` (US2); PR-style key reads from extracted archive and does NOT touch `ILocalGitClient` (US3 no-regression); deleted file in `local:staged` → `SourceUnavailable`. |
...
| `REBUSS.Pure.RoslynProcessor.Tests\StructuralChangeDetectorTests.cs` | `StructuralChangeDetector` (post-decomposition integration tests) — full before/after diff scenarios across all detected change kinds (TypeAdded/Removed, BaseTypeChanged, MemberAdded/Removed, SignatureChanged). 22 cases — the regression contract for the orchestrator's public output (Description strings, LineNumbers, sort order). |
| `REBUSS.Pure.RoslynProcessor.Tests\TypeDeclarationIndexTests.cs` | `TypeDeclarationIndex` — top-level class without namespace → bare name; namespaced types → qualified name; nested types excluded; `GetKindName` per type kind (class/struct/record/record struct/interface). |
| `REBUSS.Pure.RoslynProcessor.Tests\TypeChangeDetectorTests.cs` | `TypeChangeDetector` — added type emits TypeAdded with kind name + LineNumber; removed type emits TypeRemoved with null LineNumber; matched names emit nothing; record kind rendered correctly. |
| `REBUSS.Pure.RoslynProcessor.Tests\BaseTypeChangeDetectorTests.cs` | `BaseTypeChangeDetector` — no change emits nothing; only-added → "Implements added"; only-removed → "Implements removed"; both → "Base type changed: X → Y". |
| `REBUSS.Pure.RoslynProcessor.Tests\Members\MemberKeyExtractorTests.cs` | `MemberKeyExtractor` — `Key` distinguishes method overloads by param types; indexer with `this[T]`; ctor with `.ctor(T)`; field takes first variable; operator includes operator token. `GroupKey` collapses overloads to name only; indexers collapse across param counts. |
| `REBUSS.Pure.RoslynProcessor.Tests\Members\MemberSignatureComparerTests.cs` | `MemberSignatureComparer.AreEqual` — false on method param/modifier/type-param/return-type difference; false on property accessor-kind difference; false on field type difference; true for identical signatures and for body-only changes (signature unchanged). |
| `REBUSS.Pure.RoslynProcessor.Tests\Members\MemberChangeFormatterTests.cs` | `MemberChangeFormatter` — `FormatAdded` per kind (method with return type, property with type, field with type); `FormatRemoved` prefixes visibility (Public/Protected/Internal/none); `FormatSignatureChanged` per kind with `→` arrow; `FormatParams` truncation at >3 params ("type1, type2, ... +N"); `GetVisibility` per modifier. |
| `REBUSS.Pure.RoslynProcessor.Tests\Members\MemberChangeDetectorTests.cs` | `MemberChangeDetector` — sole-overload param edit pairs as one SignatureChanged (orphan-pairing heuristic); real overload addition does NOT pair (group has 2 → falls through to MemberAdded); matched-key signature differences (added `static`) emit SignatureChanged; property type change emits "Property changed: X : int → string"; all-members-removed emits MemberRemoved per member with null LineNumber; identical members emit nothing. |
...
// The availability gate follows the agent too: Claude never depends on the Copilot SDK starting.
if (agent == "claude")
{
...
// selector binds the per-review git ref + dedupes the workspace-root-missing warning.
services.AddSingleton<RemoteArchiveSourceProvider>();
