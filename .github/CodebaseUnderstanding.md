...
| File | Role | Depends on |
|---|---|---|
| `REBUSS.Pure\Services\CompositeCodeProcessor.cs` | `ICodeProcessor` implementation: chains all registered `IDiffEnricher` implementations in ascending `Order`; logs and skips on failure | `ICodeProcessor`, `IEnumerable<IDiffEnricher>` |

### Context Window Awareness (REBUSS.Pure\Services\ContextWindow)

| File | Role | Depends on |
|---|---|---|
| `REBUSS.Pure\Services\ContextWindow\ContextWindowOptions.cs` | `IOptions<T>` configuration: safety margin, chars-per-token, default budget, min/max guardrails, gateway cap (`GatewayMaxTokens` — optional override), model registry | — |
| `REBUSS.Pure\Services\ContextWindow\HostGatewayDetector.cs` | Static mapping from MCP `clientInfo.Name` (claude-code/claude-ai → 25 000, cursor → 24 000, codex → 20 000, default → 20 000) to per-tool-result token cap | — |
| `REBUSS.Pure\Services\ContextWindow\IGatewayBudgetState.cs` + `GatewayBudgetState.cs` | Singleton state resolving the effective gateway cap. Precedence: explicit `ContextWindow:GatewayMaxTokens` from config (when present) > lazily autodetected via `HostGatewayDetector` from `McpServer.ClientInfo.Name` resolved through DI on first access (after `initialize` handshake) > null. Result is cached after first computation. | `IOptions<ContextWindowOptions>`, `IServiceProvider`, `ILogger<GatewayBudgetState>` + `configExplicit` flag from `Program.cs` |
| `REBUSS.Pure\Services\ContextWindow\ContextBudgetResolver.cs` | Budget resolution with precedence: (1) explicit per-call `maxTokens` — bypasses gateway cap entirely; (2) model registry; (3) default fallback. Registry/default results are clamped by `IGatewayBudgetState.GetEffectiveCap()` before guardrails + safety margin. Registry lookup uses `NormalizeModelId` (lowercase, spaces/dots/underscores/hyphens → single hyphen) — tries exact normalized match then longest-prefix match (`"Claude Sonnet 4.6"` → `"claude-sonnet-4-6"` prefix-matches `"claude-sonnet-4"` → 200 K) | `IContextBudgetResolver`, `IOptions<ContextWindowOptions>`, `IGatewayBudgetState`, `ILogger<ContextBudgetResolver>` |
...
| `REBUSS.Pure\Cli\InitCommand.cs` | CLI `init` command: generates MCP config files, copies prompt/instruction files, runs auth flow; auto-detects IDEs (`DetectsVsCode`, `DetectsVisualStudio`, `DetectsClaudeCode`); supports `--ide vscode`/`--ide vs`/`--ide claude` explicit targeting; Claude Code project-scoped config is written to `.mcp.json` at the repo root and uses `mcpServers` key via `McpConfigTarget.UseMcpServersKey`; global mode (`-g`) writes to `~/.mcp.json` (VS), `%APPDATA%\Code\User\mcp.json` (VS Code), `~/.claude.json` (Claude Code, merged into existing `mcpServers` key) via `ResolveGlobalConfigTargets()`; constants: `McpConfigFileName = "mcp.json"`, `ClaudeCodeMcpConfigFileName = ".mcp.json"`, `ClaudeCodeGlobalConfigFileName = ".claude.json"`, `VsGlobalMcpConfigFileName = ".mcp.json"`, `ClaudeCodeMarkerFile = "CLAUDE.md"` |
...
| `.claude\commands\<name>.md` | *(Generated by `init` when Claude Code is detected)* Claude Code slash-command copies of the deployed prompts (`.prompt` suffix stripped) — exposes prompts as `/review-pr`, `/self-review` in the Claude CLI |
...
| `REBUSS.Pure.Tests\Cli\InitCommandTests.cs` | `InitCommand` — generates `mcp.json` (local and global `-g` mode), `--ide vscode`/`--ide vs`/`--ide claude` explicit IDE targeting, Claude Code detection (`.claude/` dir, `CLAUDE.md` file) with `mcpServers` key, copies prompt and instruction files (always overwrites), error cases, subdirectory support, Azure DevOps CLI login, GitHub CLI login, PAT carry-over, `ResolveGlobalConfigTargets` (3 targets: VS, VS Code, Claude Code); global-mode tests use nested `GlobalTestContext` disposable fixture (creates temp Git repo + simulated global config dirs), inject `globalConfigTargetsResolver` pointing to temp dirs so no real user-home files are touched |
