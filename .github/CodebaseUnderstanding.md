...
| `REBUSS.Pure\Cli\Prompts\review-pr.prompt.md` | Embedded resource: PR review prompt template (agent-neutral wording — title "AI-Assisted", references the configured agent generically). Redesigned for `get_pr_metadata`(with budget) + `get_pr_content` page loop workflow. Pattern-matches the `[review-mode: <agent>-assisted]` header to identify the actual backend (copilot/claude). Includes strict "No Repository Exploration" constraint forbidding non-MCP access. **Important**: prompt instructs the agent to surface server-returned remediation verbatim and to NEVER invent setup commands like `gh copilot setup` or `claude /login` unless the error response explicitly mentions them — this prevents Copilot/Claude lingo bleed-through. |
| `REBUSS.Pure\Cli\Prompts\self-review.prompt.md` | Embedded resource: self-review prompt template (agent-neutral, mirrors `review-pr.prompt.md`'s wording rules); redesigned for `get_local_content` page loop workflow. |
