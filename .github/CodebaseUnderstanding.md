...
| `REBUSS.Pure\Services\CopilotReview\CopilotReviewOrchestrator.cs` | Feature 013 — `PrEnrichmentOrchestrator`-style trigger/wait/snapshot lifecycle with `ConcurrentDictionary<string, CopilotReviewJob>` keyed on review key (string-based, source-agnostic). Re-paginates the enrichment result against `CopilotReview.ReviewBudgetTokens`, dispatches all page reviews **in parallel** via `Task.Run` per page + `Task.WhenAll` — the `CopilotRequestThrottle` (3-second `SemaphoreSlim` gate) serializes outgoing SDK calls while model response wait times overlap, yielding real wall-time reduction. Each page task writes to its own `pageResults[idx]` slot (no contention). Wraps each page in a 3-attempt retry loop (Clarification Q1) that fills in `FailedFilePaths` on exhaustion. `CompletedPages` incremented atomically via `Interlocked.Increment`; `CopilotReviewWaiter` polls this counter for order-agnostic progress notifications. Narrow exception to Principle VI. |
