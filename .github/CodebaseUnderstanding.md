...
| `REBUSS.Pure\Cli\Mcp\McpConfigJsonBuilder.cs` | Step 3 refactor — `internal static class McpConfigJsonBuilder` with two entry points: `Build(string normalizedExePath, string normalizedRepoPath, string? pat = null, bool useMcpServersKey = false, string? agent = null)` emits a fresh document via string interpolation of pre-escaped paths; `Merge(string existingJson, string rawExePath, string rawRepoPath, string? pat = null, bool useMcpServersKey = false, string? agent = null)` upserts the `REBUSS.Pure` server entry into existing JSON via `Utf8JsonWriter` (raw paths in, escaping done by writer), preserves all unrelated top-level keys + other server entries, and carries over an existing `--pat` arg when the caller passes null/whitespace. On invalid JSON falls back to `Build`. Private `ExtractExistingPat(JsonElement, string)` reads the carried-over PAT. Wire format unchanged from the prior `InitCommand.BuildConfigContent` / `MergeConfigContent` — same key order, same arg order (`--repo`, value, optional `--pat`, optional `--agent`). |
