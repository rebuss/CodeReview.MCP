...
public class GetPullRequestDiffToolHandlerTests
{
    ...
    public async Task ExecuteAsync_DoesNotIncludeMetadataInOutput()
    {
        ...
        // Plain text output only contains diff content and manifest, not PR metadata
    ...
    public async Task ExecuteAsync_AcceptsOptionalModelName()
    {
        ...
        var blocks = (await _handler.ExecuteAsync(prNumber: 42, modelName: "Claude Sonnet")).ToList();
    ...
    public async Task ExecuteAsync_NoBudget_NoPaginationMetadata()
    {
        ...
        // F003 path: last block is manifest, no pagination footer
        Assert.DoesNotContain("hasMore:", lastBlock);
        Assert.Contains("Manifest:", lastBlock);
    }
    ...
    public async Task ExecuteAsync_ExplicitBudget_AllFits_SinglePage()
    {
        ...
        var pageSlice = new PageSlice(1, 0, 1,
            ...
            100, 139650);
        _pageAllocator.Allocate(Arg.Any<IReadOnlyList<PackingCandidate>>(), Arg.Any<int>())
            .Returns(new PageAllocation(new[] { pageSlice }, 1, 1));
        _pageReferenceCodec.Encode(Arg.Any<PageReferenceData>()).Returns("encoded_ref");

        var blocks = (await _handler.ExecuteAsync(prNumber: 42, maxTokens: 200000)).ToList();
        var lastBlock = blocks.Cast<TextContentBlock>().Last().Text;
    ...
    public async Task ExecuteAsync_ExplicitBudget_MultiPage_HasMoreTrue()
    {
        ...
        var page1Slice = new PageSlice(1, 0, 1,
            new[] { new PageSliceItem(0, PackingItemStatus.Included, 100) },
            100, 900);
        var page2Slice = new PageSlice(2, 1, 2, Array.Empty<PageSliceItem>(), 0, 0);
        _pageAllocator.Allocate(Arg.Any<IReadOnlyList<PackingCandidate>>(), Arg.Any<int>())
            .Returns(new PageAllocation(new[] { page1Slice, page2Slice }, 2, 1));
        _pageReferenceCodec.Encode(Arg.Any<PageReferenceData>()).Returns("encoded_ref");
        ...
        Assert.Contains("next:", lastBlock);
    }
    ...
    public async Task ExecuteAsync_EmptyDiffWithExplicitBudget_ReturnsEmptyPageWithPagination()
    {
        ...
        _pageAllocator.Allocate(Arg.Any<IReadOnlyList<PackingCandidate>>(), Arg.Any<int>())
            .Returns(new PageAllocation(new[] { emptyPage }, 1, 0));
        _pageReferenceCodec.Encode(Arg.Any<PageReferenceData>()).Returns("encoded_ref");
    ...
    public async Task ExecuteAsync_PageReference_DecodesAndServes()
    {
        ...
        var pageSlice = new PageSlice(2, 0, 1,
            new[] { new PageSliceItem(0, PackingItemStatus.Included, 100) }, 100, 139650);
        _pageAllocator.Allocate(Arg.Any<IReadOnlyList<PackingCandidate>>(), Arg.Any<int>())
            .Returns(new PageAllocation(new[]
            {
        ...
        var blocks = (await _handler.ExecuteAsync(pageReference: "valid_ref")).ToList();
    ...
    public async Task ExecuteAsync_StalenessDetected_WarningPresent()
    {
        ...
        var blocks = (await _handler.ExecuteAsync(pageReference: "ref")).ToList();
        var lastBlock = blocks.Cast<TextContentBlock>().Last().Text;

        Assert.Contains("STALE:", lastBlock);
    }
    ...
    public async Task ExecuteAsync_SameSha_NoStalenessWarning()
    {
        ...
        Assert.DoesNotContain("STALE:", lastBlock);
    }
    ...
    public async Task ExecuteAsync_PageNumberDirect_ReturnsRequestedPage()
    {
        ...
        var page2 = new PageSlice(2, 0, 1,
            new[] { new PageSliceItem(0, PackingItemStatus.Included, 100) }, 100, 900);
        _pageAllocator.Allocate(Arg.Any<IReadOnlyList<PackingCandidate>>(), Arg.Any<int>())
            .Returns(new PageAllocation(new[] { page1, page2 }, 2, 1));
        _pageReferenceCodec.Encode(Arg.Any<PageReferenceData>()).Returns("encoded_ref");

        var blocks = (await _handler.ExecuteAsync(prNumber: 42, maxTokens: 200000, pageNumber: 2)).ToList();
        var lastBlock = blocks.Cast<TextContentBlock>().Last().Text;

        Assert.Contains("Page 2 of", lastBlock);
    }
